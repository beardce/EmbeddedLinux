...
int main()
{
    ...
    int xold = 0, yold = 0;
    long int location = 0;
    int lineR = 0, lineG = 17, lineB = 0, lineRadius = 0;
    ...
    printf("Enter line radius in pixels (line width = 2*line radius +1): ");
    scanf("%d", &lineRadius);
    printf("Enter how red you want your line to be(0-31): ");
    scanf("%d", &lineR);
    if((lineR > 31) | (lineR < 0)){
        printf("Invalid input, setting red level to 10\n");
        lineR = 10;
    }
    printf("Enter how green you want your line to be(0-63): ");
    scanf("%d", &lineG);
    if((lineG > 63) | (lineG < 0)){
        printf("Invalid input, setting green level to 10\n");
        lineG = 10;
    }
    printf("Enter how blue you want your line to be(0-31): ");
    scanf("%d", &lineB);
    if((lineB > 31) | (lineB < 0)){
        printf("Invalid input, setting blue level to 10\n");
        lineB = 10;
    }
	...
	while(rc_get_state() != EXITING) {
        ...
        if((x != xold) || (y != yold)) {
            ...
            // Set old location to user defined color
            unsigned short int t = lineR <<11 | lineG << 5 | lineB;
            for(int i=-lineRadius; i <= lineRadius; i++){
                for (int j=-lineRadius; j<= lineRadius; j++){
